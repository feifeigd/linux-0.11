...
#define LATCH (1193180/HZ)	// 每个时间片的震荡次数，即系统每10毫秒发生一次时钟中断
...
static union task_union init_task = {INIT_TASK,};	// 进程0的 task_struct
...
struct task_struct * task[NR_TASKS] = {&(init_task.task), };	// 初始化进程槽task[NR_TASKS]的第一项为进程0，即task[0]为进程0占用
...
void sched_init(void)
{
	...
	p = gdt+2+FIRST_TSS_ENTRY;	// 从GDT的6项，即TTS1开始向上全部清零，并且将进程槽从1往后的项清空，0项为进程0所用
